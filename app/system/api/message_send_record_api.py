...
# @router.get("/u/message_send_record/list")
# async def umessage_send_record_list(request: Request,current_user: str = Depends(get_current_user)):
#     PageIndex = int(request.query_params.get("PageIndex", 1))
#     PageLimit = int(request.query_params.get("PageLimit", 10))
#     dal = MessageSendRecordDal(request.state.db)
...
# @router.post("/u/message_send_record/add")
# async def umessage_send_record_add(request: Request,current_user: str = Depends(get_current_user)):

#     dal = MessageSendRecordDal(request.state.db)
#     data =await dal.AddByJsonDataUser(await request.json())
#     return Result.success(data)

# @router.post("/u/message_send_record/update")
# async def umessage_send_record_update(request: Request,current_user: str = Depends(get_current_user)):
#     dal = MessageSendRecordDal(request.state.db)
#     data =await dal.UpdateByJsonDataUser(await request.json())
#     return Result.success(data)

# @router.post("/u/message_send_record/save")
# async def umessage_send_record_save(request: Request,current_user: str = Depends(get_current_user)):
#     dal = MessageSendRecordDal(request.state.db)
#     jsonData = await request.json()
#     if jsonData.get('id'):
...
# @router.get("/u/message_send_record/delete")
# async def umessage_send_record_delete(id:int,request: Request,current_user: str = Depends(get_current_user)):
#     dal = MessageSendRecordDal(request.state.db)
#     data =await dal.DeleteByUser(id)
#     return Result.success("删除成功")
        
# @router.get("/u/message_send_record/get")
# async def umessage_send_record_get(id:str,request: Request,current_user: str = Depends(get_current_user)):
#     dal = MessageSendRecordDal(request.state.db)
